...
Структура:
В программе частично используются методы многопоточного программирования, класс Figure отражает общие для всех артефактов поля и методы,
а его дочерние классы Circle, Rectangle, Triangle реализуют поля конкретных альтернатив. После этого элементы помещаются в массив,
который обрабатывается специальной функцией, для сортировки артефактов в необходимом порядке.

Использование:
Для использования необходимо запустить созданный cmake файл из командной строки указывая при этом дополнительные параметры. Первый дополнительный
...
указать второй параметр, путь к .txt файлу с тестовыми значениями.
...
Тесты:
При разработке применялось 5 тестов с 0, 3, 5, 10, 20 артефактами соответственно.